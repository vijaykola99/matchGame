...
class App extends Component {
  ...
  renderGameView = () => {
    const {category, imgUrl} = this.state
    return (
      <div className="images-container">
        <img className="main-image" src={imgUrl} alt="images" />
        <ul className="tabs-container">
          {tabsList.map(eachTab => (
            <li key={eachTab.tabId}>
              <button
                onClick={() => {
                  this.onsetActiveCategory(eachTab.tabId)
                }}
                className={`tab-button ${
                  eachTab.tabId === category ? 'active-tab' : null
                }`}
                type="button"
              >
                {eachTab.displayText}
        ...
        <ul className="thumbnails-containers">
          {imagesList
            .filter(eachItem => eachItem.category === category)
            .map(eachImage => (
              <li key={eachImage.id}>
                <img
                  onClick={() => {
                    this.imageClick(eachImage.thumbnailUrl)
                  }}
      ...
      <div className="images-container">
        <div className="score-card-container">
          <div className="result-score-container">
            ...
            >
              <img
                className="retry-logo"
                src="https://assets.ccbp.in/frontend/react-js/match-game-play-again-img.png"
                alt="reset"
              />
              <span className="score-text">Play Again</span>
            </button>
          </div>
        </div>
      </div>
  ...
  render() {
    const {isTrue, score, time} = this.state
    return (
      <div className="app-background">
        <nav className="nav-bar">
          <img
            ...
            alt="website logo"
          />
          <div className="timer-display-text-container">
        ...
        {isTrue ? this.renderScoreCardView() : this.renderGameView()}
      </div>
    )
  }
}
